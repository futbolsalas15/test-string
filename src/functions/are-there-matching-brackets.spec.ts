...
describe("are-there-matching-brackets.ts", () => {
  it("should return true if the text contains matching brackets, using different bracket types", () => {
    expect(areThereMatchingBrackets("[[test]]", BracketType.Square)).toEqual(
      true
    );
    expect(
      areThereMatchingBrackets(
        "<a test inside<nested> angle>",
        BracketType.Angle
      )
    ).toEqual(true);
    expect(
      areThereMatchingBrackets("(another (nested) test)", BracketType.Round)
    ).toEqual(true);
    expect(
      areThereMatchingBrackets("{another {{nested}} test}", BracketType.Curly)
    ).toEqual(true);
  });
  ...
  it("should return true if non-bracket characters are ignored appropriately", () => {
    expect(areThereMatchingBrackets("{abc...xyz}", BracketType.Curly)).toEqual(
      true
    );
  });
  ...
  it("should return true if there are some open brackets and multiple close brackets, but all open brackets close", () => {
    expect(areThereMatchingBrackets("{{}}}}}}", BracketType.Curly)).toEqual(
      true
    );
  });
});
